...
buyers.put("/me", authMiddleware, async (c) => {
  ...
  const result = await c.env.D1.prepare("UPDATE buyers SET full_name = ?, address = ?, phone = ?, image_id = ?, updated_at = current_timestamp WHERE user_id = ?")
    .bind(full_name, address || null, phone || null, imageId, payload.userId)
    .run();
...
buyers.put("/:id", adminMiddleware, async (c) => {
  const id = c.req.param("id");
  const { user_id, full_name, address, phone } = await c.req.json();
  const result = await c.env.D1.prepare("UPDATE buyers SET user_id = ?, full_name = ?, address = ?, phone = ?, updated_at = current_timestamp WHERE id = ?")
    .bind(user_id, full_name, address || null, phone || null, id)
    .run();
