...
sellers.post("/", async (c) => {
  ...
  const result = await c.env.D1.prepare("INSERT INTO sellers (user_id, store_name, description, contact_phone) VALUES (?, ?, ?, ?)").bind(user_id, store_name, description || null, contact_phone || null).run();
  return c.json({ id: result.meta.last_row_id });
});

sellers.put("/me", authMiddleware, async (c) => {
  const payload = c.get("jwtPayload");
  const { store_name, description, contact_phone } = await c.req.json();
  const result = await c.env.D1.prepare("UPDATE sellers SET store_name = ?, description = ?, contact_phone = ?, updated_at = current_timestamp WHERE user_id = ?").bind(store_name, description || null, contact_phone || null, payload.userId).run();
